...
heroImage: '/images/window-obj.webp'
tags: 'javascript DOM'
...
There are many useful methods and objects it provides, but some are just more frequently used than others, which is the reason of me writing this article to introduce you to the features that I found most useful to me. 
...
In the Above Example, the setInterval method updates our UI each second with an incremental value, thus forming a Timer.
...
```
There's also a `localStorage.removeItem(key)` which removes the item from the storage by its key.
